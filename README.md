...
统一控制协议从 `v0.21.0-rc1` 起由独立 `willdeep-runtime-protocol` crate 定义。`v0.21.0-rc5` 已覆盖 Workspace/Session/Turn/Agent/Event/Task/Approval/Question 公开 DTO、TUI 事件与 Inbox/控制、TUI/Web Workspace bridge，以及跨重启 Request ID 幂等日志。运行 `willdeep daemon capabilities` 可读取受 Token 保护的协议版本、对象类型、操作名、传输方式与大小限制。完整契约见 [`docs/RUNTIME_CONTROL_API.md`](docs/RUNTIME_CONTROL_API.md)。
