...
统一控制协议从 `v0.21.0-rc1` 起由独立 `willdeep-runtime-protocol` crate 定义。`v0.21.0-rc9` 已覆盖 Workspace/Session/Turn/Agent/Event/Task/Approval/Question、附件与 Diff Review 公开 DTO；TUI 的事件、Inbox、控制、Workspace、Diff Center、搜索与 Turn，以及 Web Session/Turn 管理均通过共享 Runtime Client，修改请求使用跨重启 Request ID 幂等日志。本机客户端优先使用权限收紧的 Unix Socket 或仅限本机的 Windows Named Pipe，旧状态仍兼容回环 TCP。运行 `willdeep daemon capabilities` 可读取受 Token 保护的协议版本、对象类型、操作名、真实传输方式与大小限制。完整契约见 [`docs/RUNTIME_CONTROL_API.md`](docs/RUNTIME_CONTROL_API.md)。
