...
统一控制协议从 `v0.21.0-rc1` 起由独立 `willdeep-runtime-protocol` crate 定义。`v0.21.0-rc14` 已覆盖全部 11 类公开对象 DTO。Tool Activity 和 Workspace Change Artifact 已被 TUI 与 Web 直接消费；共享 Rust Client 提供 `tools/tool/artifacts/artifact` 类型化查询，调用方无需手写操作名和返回类型。具体变化路径与内容仍须通过受授权的精确 Diff API 获取。本机客户端优先使用 Unix Socket 或 Windows Named Pipe。完整契约见 [`docs/RUNTIME_CONTROL_API.md`](docs/RUNTIME_CONTROL_API.md)。
