...
统一控制协议从 `v0.21.0-rc1` 起由独立 `willdeep-runtime-protocol` crate 定义。`v0.21.0-rc6` 已覆盖 Workspace/Session/Turn/Agent/Event/Task/Approval/Question 与 Diff Review 公开 DTO；TUI 的事件、Inbox、控制、Workspace 和 Diff Center 均通过共享 Runtime Client，修改请求使用跨重启 Request ID 幂等日志。运行 `willdeep daemon capabilities` 可读取受 Token 保护的协议版本、对象类型、操作名、传输方式与大小限制。完整契约见 [`docs/RUNTIME_CONTROL_API.md`](docs/RUNTIME_CONTROL_API.md)。
