...
统一控制协议从 `v0.21.0-rc1` 起由独立 `willdeep-runtime-protocol` crate 定义。`v0.21.0-rc18` 已覆盖全部 11 类公开对象 DTO。共享 Rust Client 提供 Workspace、Session、Agent、Turn、Task、Approval、Question、Tool、Artifact 与 Event 的高频类型化观察/控制方法；TUI Runtime bridge 已迁移这些方法。Web Runtime 侧栏展示当前 Workspace 的 Agent、待审批/回答、关注项、Tool 与 Artifact 摘要，且不下发 Agent Workspace、报告或内部错误。仓库提供固定跨语言兼容夹具。完整契约见 [`docs/RUNTIME_CONTROL_API.md`](docs/RUNTIME_CONTROL_API.md)。
