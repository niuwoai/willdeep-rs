...
const ASSISTANT_VOICE_PREFIXES: &[&str] = &[
    ...
    "here's",
    "here is",
    // 不收光秃秃的「はい、」「では、」：日文里用户答「是」、说「那就…」也这么开口
    // （「はい、コミットして」「では、PRを作って」），2026-09-21 实弹评测里前者把
    // 日文样本全部误杀。只收助手才会接的那半句。与 Xedit 1.394.0-rc3 同一份名单。
    "はい、承知",
    "はい、かしこまり",
    "承知しました",
    "かしこまりました",
...
mod tests {
    ...
    fn japanese_yes_is_the_user_answering_not_the_assistant_speaking() {
        ...
        assert_eq!(
            sanitize("では、PRを作って").as_deref(),
            Some("では、PRを作って")
        );
        assert_eq!(
            sanitize("はい、ではコミットして").as_deref(),
            Some("はい、ではコミットして")
        );
    }
