...
impl PluginManifest {
    pub fn parse(source: &str) -> Result<Self, ManifestError> {
        ...
        for item in array_of(contributes.get("destinations"), "contributes.destinations")? {
            manifest.destinations.push(parse_destination(item, &mut unsupported)?);
        }
        for item in array_of(contributes.get("sidebars"), "contributes.sidebars")? {
            manifest.sidebars.push(parse_sidebar(item, &mut unsupported)?);
        }
        ...
        for item in array_of(contributes.get("commands"), "contributes.commands")? {
            manifest.commands.push(parse_command(item, &mut unsupported)?);
        }
        ...
        for item in array_of(contributes.get("settings"), "contributes.settings")? {
            manifest.settings.push(parse_setting(item, &mut unsupported)?);
        }
...
mod tests {
    ...
    fn the_permission_vocabulary_matches_the_macos_host() {
        ...
        assert_eq!(HostAction::parse("session.open"), Some(HostAction::SessionOpen));
    }
    ...
    fn a_permission_this_host_lacks_does_not_block_installation() {
        ...
        assert!(manifest.permissions.contains(&PluginPermission::WorkspaceRead));
    ...
    fn a_menu_location_this_host_lacks_is_dropped_not_fatal() {
        ...
        assert!(manifest.menus.contains_key(&PluginMenuLocation::CommandPalette));
        assert!(
            manifest
                .unsupported
                .iter()
