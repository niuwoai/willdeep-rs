...
> 最后更新：2026-08-25 | 当前版本：v0.43.0-rc3
...
- 空白新会话的即时工作区欢迎引导；
- 多行 Prompt 编辑使用同一套 Unicode 单元格规则完成换行、滚动、鼠标定位与光标渲染；支持 F2 临时展开为终端大输入空间，并保留文本粘贴附件和可删除图片附件；图片可用 `Ctrl+V`、`Ctrl+Shift+V` 或终端可上报的 `Cmd+V` 从本机系统剪贴板附加；
