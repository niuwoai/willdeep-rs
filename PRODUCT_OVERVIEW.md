...
> 最后更新：2026-09-27 | 当前版本：v0.84.0-rc1（验收记录见 docs/AGENT_RELIABILITY_WORK.md；未发布）
...
- Web 插件宿主提供与 macOS 版同名同序的桥 2.6.0 能力（文件、命令、代发网络、结构化存储、技能清单、生图、问模型时附插件媒体目录里的图片、把文本递给主 Agent、宿主事件；视频附件与流式思考两项仅 macOS 版提供），共享插件包零改动即可在两端运行；本宿主认不出的清单词汇降级显示为「本宿主不支持」而不是拒装；需要原生文件框的插件在 Web 上改由浏览器选文件并上传（参照图 ≤20 MiB；短剧分集背景音乐 ≤200 MiB，v0.84.0-rc1 起）。详见 docs/PLUGINS.md。
- 插件 MCP 与 macOS 版同一套机制（v0.83.0-rc1）：`willdeep web` 在本机开插件 MCP 网关（`mcp-gateway.json`，端口与 token 固定），外部客户端连上即可按需拉起插件，不必先打开插件页；插件进程可反向请求宿主出图、问模型；已启用插件的 MCP 工具进聊天；插件页标题栏、图标与主题变量对齐 macOS。
...
- TUI 聊天区实时落下模型的中途文字与每次工具调用（`· … / ✓ / ✗ 工具 · 摘要`，完成后原地改标记；早先的小轮次收成一行汇总、最新的小轮次一行汇总加最新一条，`/tools` 展开），正文流式增量走临时预览行（「回复中」），思考型模型的思维链只占最后 3 行灰色滚动显示（「思考中」）、正文一开始即隐藏；所有进聊天区的文字先做终端安全处理（制表符展开、控制字符转义），避免残影；会话重开后工具行按记录回放（投影带每条调用的名字、脱敏摘要与有无结果，Web 回放成运行卡片）；系统提示词要求模型在任务开始前和阶段切换时用一句话汇报进度；轮次结束落一行「── 本轮结束 · … · 轮到你 ──」分隔线并响铃，输入框标题常驻「本轮进行中 · 回车即送达 / 轮到你」状态；本轮在跑时按回车先把话直接送进正在跑的这一轮（下一次调模型前以用户身份注入，不打断手上的活），送不到才排队并以「待发」标记回显、发出时改回；聊天标题、活动区和底部状态栏一致的动态运行指示、当前阶段、累计耗时、无新事件等待提示与 Runtime 重连恢复；Web 提供单行工作状态、逐轮工具轨迹（与 TUI 同一套折叠规则：早先的小轮次一行汇总，最新一轮汇总加最新一步，外层再按总步数兜底）、灰色的思维链尾巴行（最后 240 字，正文或工具一来即隐藏）、停止生成，本轮进行中的插话送达（回车经 `/api/sessions/{id}/steer` 转 `turn.steer`，没有在途任务时提示稍后再发），轮次结束落「── 本轮结束 · 总耗时 · 工具 N 次 · 轮到你 ──」分隔线，输入框下方提示常驻「本轮进行中 / 轮到你」状态；
...
- macOS 桌面会话桥接保留 assistant 工具调用与 tool 结果的关联 ID；Provider 重放前会清理旧版本遗留的孤立工具记录，使已经导入的坏会话也能继续使用；
