...
> 最后更新：2026-09-20 | 当前版本：v0.78.0-rc8（验收记录见 docs/AGENT_RELIABILITY_WORK.md；未发布）
...
- TUI 聊天区实时落下模型的中途文字与每次工具调用（`· … / ✓ / ✗ 工具 · 摘要`，完成后原地改标记；连续超过 6 条自动收成一行汇总只留最新 4 条，`/tools` 展开），正文流式增量与思考型模型的思维链都走临时单行预览（标签「回复中」/「思考中」）；系统提示词要求模型在任务开始前和阶段切换时用一句话汇报进度；轮次结束落一行「── 本轮结束 · … · 轮到你 ──」分隔线并响铃，输入框标题常驻「本轮进行中 / 轮到你」状态，本轮在跑时排队的提示词以「待发」标记回显、发出时改回；聊天标题、活动区和底部状态栏一致的动态运行指示、当前阶段、累计耗时、无新事件等待提示与 Runtime 重连恢复；Web 提供单行工作状态、逐轮工具轨迹与停止生成；
