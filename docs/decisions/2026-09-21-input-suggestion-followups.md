...
### 后续（同日，0.78.0-rc31 → 0.79.0-rc1）

- Xedit 1.394.0-rc3 同步了预测的两处修复：「做完未收口照常预测、收口不给客套话」，并补上日文助手口吻拦截。已合入本地 develop，未推送。
- rs 侧：日文「では、」不再误杀（rc31）；弹窗改为固定配色（rc32）；选过字后点输入框能切焦点、能打字（rc33）；轮次进行中可以 `/model`、Runtime 比客户端旧且空闲时自动升级（0.79.0-rc1）。
- 评估后不做：把 verifier 的审查挪到子 agent 写完脚本之后。判官只看命令字符串；若要把脚本内容喂给判官，又会打开提示注入通道，这是安全边界上的取舍，留给 rocky 决定。
- 待核实：`docs/TUI_GUIDE.md` 说"有任务等待审批时 Runtime 不会退出"，upgrade 代码却提示这类任务"will be dropped by this handoff"。自动升级对这两种情况都会跳过，但哪一边是对的还没核实。
