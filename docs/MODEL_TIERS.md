...
| 档 | 窗口 | 角色 | 私有化难度 | some.im 档位 |
|---|---|---|---|---|
| **S（worker）** | 32K–64K | 可验证的有界任务：修测试、修编译、定位、日志解释、模板生成、命令编排 | 单卡，容易 | `someim-32b-<工种>`（已建成） |
| **M（standard）** | ~256K | **会话默认档**：日常编码循环、单模块开发、评审、文档、单章创作 | 多卡，可行 | 会话主模型（现 `someim-auto-flash` 等） |
| **L（deep）** | 1M | 跨模块重构、全库理解、超长材料（大日志全量、多文档综述、长篇规划） | 极难，多数场景没有 | 待建：`someim-1m`（网关虚拟模型，路由到当下最大窗口的可用上游） |
...
| `someim-1m` / `someim-256k` 网关虚拟模型 | ⏳ 待网关配置——2026-08-16 实测两个名字都返回 `model_not_configured`。建法同 `someim-32b`：虚拟模型 + `target_models` 指向对应窗口的上游，客户端零改动 |
