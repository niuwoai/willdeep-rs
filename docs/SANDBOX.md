...
**工作区所属仓库的 git 目录也自动放行**，哪怕它在工作区外。启动时跑一次
`git rev-parse --git-dir --git-common-dir`，落在工作区外的结果加进可写根：

- 子模块：`.git` 是个文件，指向父仓库的 `.git/modules/<name>`；
- `git worktree`：指向主仓库的 `.git/worktrees/<name>`，对象库在主仓库 `.git`；
- 在仓库子目录里开会话：`.git` 在上层。

不放行的话这几种场景下 commit、fetch、改 ref 全被拦，围栏开着就提交不了代码。
放行的只是这个工作区自己的仓库元数据——普通仓库的 `.git` 本来就在工作区里可写，
没有多出新的风险面。不在仓库里、没装 git 时什么都不加。

可写根在**会话启动时**算好；改了 `sandbox_writable_roots` 要重启会话才生效。
...
后半段写死「只有两条路」，是因为旧版只说「请用户放宽工作区策略」，模型就用
`ask_user` 问一句「要不要放宽」，用户点了同意，围栏纹丝不动——问答只是一段文字，
改不了档位，也改不了已经算好的可写根。
