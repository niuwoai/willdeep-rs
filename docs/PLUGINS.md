...
`~/.willdeep/plugin-media/<id>/upload-<uuid>.<ext>` → 按交付方式把落地的
**服务端绝对路径**交回插件：

- **合成**（`video.pick_reference`）：工具本身只回选中的路径，宿主直接合成
  `{"ok":true,"path":…}`，外面照样包一层 MCP 的 `content[0].text`，不进 MCP 服务。
  上传件就是选择结果，留着给后续工具读。插件一行不用改。
- **转交**（`episode.import_music`）：工具选完还要干活（拷成 `music-<hex>.<ext>`、
  写进成片设置），宿主把路径作为 `path` 参数转给原工具，结果原样回页面；调用结束后
  （不论成败）删掉上传件。插件要认 `path` 参数，没带时才弹 `osascript`。

要接管哪些工具是一张可声明的表（`plugin_web.rs` 的 `FILE_PICKER_TOOLS`）：每行是
`(插件 ID, MCP 服务, 工具名)` 加扩展名白名单、大小上限（与插件自己的上限同值）和
交付方式，新增插件只加一行。快照的 `file_pickers` 把白名单与上限带给页面，页面据此
设置文件框的 `accept` 并在上传前拦超限文件。

上传口 `POST /api/plugins/{id}/files?command=<命令 ID>&name=<原文件名>`，请求体是
文件原始字节：服务端按那条命令核类型与大小，边收边写 `.part`，收全才改名，被拒不留
残件。不走 JSON——全站 1 MiB 的请求体上限只管 JSON 接口，流式读 body 不受它限，
上限改在这里按命令核。

宿主侧会校验回来的路径确实落在这个插件的媒体目录里——页面自报一个 `/etc/passwd`
就能把任意文件喂给后续工具，这道门关在服务端。转交型另外只收 `upload-*`：媒体目录
里插件自己的成品路径合法，但宿主转交完要删它，所以不收。

只有经插件页命令（`commands.execute`）的调用会被接管；聊天里模型直接调、外部 MCP
客户端经网关调这些工具，仍按插件自己的逻辑走（弹 `osascript`）。
