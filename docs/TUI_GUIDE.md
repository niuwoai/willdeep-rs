...
| `←` / `→`、`Home` / `End` | 移动编辑光标 |
| `Ctrl+V` / `Ctrl+Shift+V` 或 `Cmd+V` | 从本机系统剪贴板附加图片；终端截获系统粘贴键时使用 `Ctrl+V` |
...
`Ctrl+V` / `Ctrl+Shift+V` / `Cmd+V` 读取的是**运行 WillDeep 那台机器**的系统剪贴板。终端截获系统粘贴键时请用 `Ctrl+V`。SSH 会话中，远端进程读不到你本地电脑的剪贴板图片——这是终端的边界，WillDeep 会明确报错，不会静默伪装成功。
