...
Worker **跟随父会话的完全访问**（0.80.0-rc1 起）：父会话此刻处于 `full-access`，Worker 的命令就免审放行、写入围栏摘掉、派工时的 verifier 也不再请判官——和主 Agent 在完全访问下一样。跟随是实时的：父会话切回别的档位，Worker 从下一次工具调用起回到下文所述的本工种策略。

不随之放宽的是**工种的职责边界**，它们不是审批，是任务包给的范围：工具面（只读工种依旧没有写工具）、声明的写集合、「只许跑 verifier」的收窄、只读 git 的收窄。破坏性形状（`rm -rf`、`git push --force`…）也和主 Agent 一样仍然弹卡问人；派工那一刻的 verifier 没有审批卡可弹，破坏性形状直接拒绝。放行记审计来源 `full-access`。

### 子 Worker 命令审核与人类兜底

以下是父会话**不在** `full-access` 时的策略。
