...
`Alt+V` / `Ctrl+V` / `Ctrl+Shift+V` / `Cmd+V` 读的是**运行 WillDeep 那台机器**的系统剪贴板。若终端截获 `Cmd+V` 或 `Ctrl+Shift+V`，请用会直接送给 TUI 的 `Alt+V` 或 `Ctrl+V`。远端进程读不到你本地电脑的剪贴板，这是终端的边界。WillDeep 会明确报错而不是静默失败。
