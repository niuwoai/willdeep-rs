...
- **Windows 上两条中断恢复测试改为仅 Unix。** `local_interrupted_write_resumes_without_replay` 和 `interrupted_foreground_child_is_discovered_resumed_and_reported_without_replay` 用 `scripts/lib/agent_eval_process.rb` 做中断控制器，这个脚本依赖进程组、fd 3/4 私有管道、按组 KILL，全是 POSIX 专有的；评测脚本本身也只在 Linux CI 上跑。所以这两条标 `#[cfg(unix)]`，断言不变。代价是：Windows 上「中断后恢复不重放」这条路径目前没有自动化覆盖。
