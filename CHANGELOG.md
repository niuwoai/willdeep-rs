...
- **TUI 的 `/mobile` 变成遥控器。** 侧栏「移动中继」显示 Runtime 报的状态（关闭 / 已连接 / 重连中、手机在线与否），不再只反映这个终端开没开过 `/mobile`。手机消息不进 TUI 的键盘队列，直接进 Runtime 的轮次队列；「待发队列」挪到侧栏「运行状态」一节。
- **手机发起的轮次，同会话的 TUI 也会弹审批。** 审批「发起端优先」只弹回发起端，但手机是遥控器不是工作台：人回到电脑前得能在终端里答。两边都弹是安全的，谁先答算谁的。

### Fixed
- **两个终端同时开 `/mobile` 会串台，还会互相刷屏。** 凭据全局共用，两个进程进同一个 room；中继是广播，A 回的 `ack` 到了 B 被当未知命令回 `error`，这条 `error` 回到 A 又被回一条 `error`，无限往返。中继挪进 Daemon 后一台机器只有一个连接；网关也只认手机命令，回声、别的桌面端的回复（包括与命令同名的 `workspace.list` 回复）一律丢弃。
- **Android 的对话面板每 5 秒被清空一次。** Android 每 5 秒发一次 `session.list` 当心跳，拿快照整体替换对话列表，而旧快照的 `messages` 永远是空的。现在快照带着选中会话的历史与实时尾巴。
- **审批在别处答掉后，TUI 里那张对话框还挂着，按下去才报 `Runtime interaction is no longer pending`。** 现在快照里 gate 一消失，背后等回答的任务就退出，对话框在一秒内自动收起，提示「这条审批已在其他端处理」；已经答了、请求还在路上的不受影响。

### Security
- 手机中继现在能看整个 Runtime、给任何会话发消息，token 泄露的影响面随之变大：写操作收窄到上面四件；新建会话不能顺手登记新目录；上中继前消息正文与审批描述里疑似凭据的词打码（保留换行与缩进，不把 Markdown 压扁）。中继服务端看得到明文的边界与 macOS 版相同，没有变化；端到端加密另立项。
...
### Fixed
- **侧栏「需要关注」里，早已过去的 Runtime 任务一直顶着「需要你处理」。** 过期规则只回收「已完成」「已取消」（5 分钟），「部分完成」「失败」「中断」的任务永不回收——同一会话每追问一轮，Inbox 就多压一条，几天前的 `部分完成 · 62h` 一直挂着，点开也无事可做。现在两条规则：同一会话里之后又提交过轮次的，上一轮的未竟任务直接退出 Inbox（用户已经接着往下聊了）；没被接续的也只保留 24 小时，与后台命令的失败保留期同一口径。
- **结束的任务耗时按「现在」算。** 三天前停下的任务显示「62h」，看着像还在跑；现在算到结束时间为止。
- **Runtime 任务在 Inbox 里被标成「后台命令」。** 它是一整轮对话，不是一条 shell 命令；列表行和详情弹窗改标「任务」。
...
- **Windows 上捕获 `willdeep run` 输出的调用方永远等不到结束。** `willdeep run` 自动拉起 Runtime Daemon 时，Windows `CreateProcess` 会把父进程所有可继承句柄复制给子进程，其中包括调用方交给 `willdeep run` 的 stdout / stderr 管道——哪怕 Daemon 的 stdio 已经重定向到日志文件。常驻的 Daemon 攥着管道写端，`willdeep run` 自己退出了，调用方（CI 脚本、`subprocess.run(capture_output=True)`、`| jq`）却读不到 EOF，一直卡住；在终端里直接运行不受影响。拉起 Daemon 前先把本进程三个标准句柄的继承标志摘掉（`SetHandleInformation`），失败时报出带上下文的错误。`configure_detached` 挪到 `daemon/detached_spawn.rs`（`daemon.rs` 已近 3000 行上限）。在 Windows CI 上逐条单独跑定位：卡住的 13 条测试全都走常驻 Runtime 路径，`--local` 和 Web 路径全部通过。
- **Windows 上并发连 Runtime 会随机报「error sending request」。** Windows 的本地传输是 Named Pipe，每个实例只服务一个客户端。Runtime 以前只挂一个等待实例，被连上之后才建下一个；这段空窗里再来的连接拿到 `ERROR_PIPE_BUSY`，reqwest 的 Named Pipe 连接器不重试，请求直接失败。事件流、状态查询和正在跑的 turn 同时存在时（例如 `willdeep daemon upgrade` 交接期间查状态）就会撞上。现在 Runtime 始终挂 4 个等待中的实例，一个被占用就补一个（`daemon/local_transport.rs`）。

**CI 停跑期间积下的失败清零。** 0.80.0-rc2 让 `ci.yml` 重新起 job 之后，暴露出约 2026-08-10 以来没人看到的几处红：
- **`headless_runtime.rs` 超过 3000 行上限。** 3015 行，`check_source_size.rb` 失败。后台 supervisor 的两条测试和它专用的辅助函数（起 supervisor、发帧请求、各平台的打印 / 等待命令、`process_exists`）自成一块，按既有 `#[path]` 子模块的做法移到 `tests/headless_runtime/background_supervisor.rs`；测试一条没少（仍是 27 条），断言未改。主文件降到 2855 行。
- **`willdeep-runtime-client` 的四条 Unix socket 测试在 Linux 上 panic。** 测试根目录写死为 `/private/tmp/...`——这个路径只在 macOS 存在，Linux 上 `create_dir_all` 直接 `PermissionDenied`，与交叉编译容器无关。改用 `/tmp`：两个平台都有，且足够短，不会碰到 `sun_path` 长度上限（这也是当初不用 macOS 很长的 `$TMPDIR` 的原因）。断言未改。
- **CI 矩阵关掉 fail-fast。** `test` 与 `linux-architectures` 两个矩阵原先任一平台失败就取消其余平台，macOS / Windows 的 job 因此一直在装前端依赖时被取消，看不到它们自己的结果。现在各平台独立跑完。同时给 `test` 与 `linux-architectures` 加 `timeout-minutes: 60`：Windows 上曾有测试卡住，job 空等到 GitHub 缺省的 6 小时才被取消。
- **aarch64 测试改在原生 arm64 runner 上跑。** 以前在 x86 runner 上经 cross 容器 `ghcr.io/cross-rs/aarch64-unknown-linux-gnu:0.2.5`（Ubuntu 16.04，git 2.7）用 qemu 跑，不认 `git init -b` 与 `git status --porcelain=v1`，27 条涉及 git 的测试全部失败——是测试环境的 git 比产品要求的还老，不是代码问题。`linux-architectures` 矩阵改为每个 target 配同架构 runner（aarch64 → `ubuntu-24.04-arm`），测试与断言不变。
- **Windows 上 7 条测试失败，都是测试写法或环境问题。** ① 4 条逐字节比对 git 恢复结果的断言（diff review 安全回退、worktree 清理 / 复核、handoff 接收）拿到 `"x\r\n"`：runner 沿用 Git for Windows 缺省的 `core.autocrlf=true`，检出时改了行尾。handoff 的接收端是产品代码 clone 出来的，没法在测试仓库里局部关掉，CI 在 Windows 上跑测试前设 `core.autocrlf false`；本地在 Windows 上跑这几条测试也需要同样的设置。② 沙箱可写根目录的配置测试把 `/opt/cache` 当绝对路径，Windows 上不是，改用 `C:\opt\cache`。③ 两条围栏测试（submodule / worktree 的 git 目录）把 `canonicalize()` 得到的 `\\?\C:\...` 路径传给 `git submodule add` / `git worktree add`，git 不认；改传相对路径，期望路径逐段 `join`（verbatim 路径里 `/` 不是分隔符）。断言内容都没改。
- **Windows 上两条中断恢复测试改为仅 Unix。** `local_interrupted_write_resumes_without_replay` 和 `interrupted_foreground_child_is_discovered_resumed_and_reported_without_replay` 用 `scripts/lib/agent_eval_process.rb` 做中断控制器，这个脚本依赖进程组、fd 3/4 私有管道、按组 KILL，全是 POSIX 专有的；评测脚本本身也只在 Linux CI 上跑。所以这两条标 `#[cfg(unix)]`，断言不变。`monitor_events` 那条同理：给 monitor 的命令是 POSIX shell（`for` 循环加 `grep --line-buffered`），Windows 上 Shell 工具走 PowerShell；monitor 自己的单元测试也只在 Unix 上跑。代价是：Windows 上「中断后恢复不重放」这条路径目前没有自动化覆盖。
- **后台 supervisor 测试的 Windows 命令一直把 PID 写错了文件。** `Set-Content -NoNewline supervisor-child.pid $child.Id` 在 Windows PowerShell 5.1 里，开关写在位置参数前面会打乱位置参数的绑定：实测在 Windows runner 上写出的是名为 `1` 的文件，`supervisor-child.pid` 从来没出现，测试等不到它，给多久都没用。改为命名参数 `Set-Content -Path … -Value … -NoNewline`。另外，等不到 PID 文件时现在会先断开 supervisor、收集它的输出再报错；以前只报「condition was not met」，看不出是慢还是命令出错，这次就是靠这个定位的。期限仍是 5 秒，「父进程断开后 5 秒内收掉子进程」的断言不变。
- **`willdeep-core` 在 Windows 上的 6 条测试：测试命令只按 POSIX shell 写。** 这个测试二进制以前在 Windows 上一次都没跑到过（前面的二进制先挂了）。① 两条 hook 测试用 `cat > file` / `cat >> file` 收 stdin，Windows 上 hook 走 PowerShell，`cat` 是 Get-Content、不读 stdin，写出空文件；`credentials_never_reach_the_hook` 因此在 Windows 上一直是空洞地通过（空文件当然不含凭据）。Windows 分支改用 `[Console]::In.ReadToEnd()` 加 .NET 文件 API。② 两条用 `ruby -e 'File.open("…")…'` 的测试：Windows PowerShell 5.1 给外部程序传参时不转义内嵌双引号，Ruby 收到的是语法错误；改用 `%q(…)`，同一条命令两个平台都成立。③ 全权档审批测试的 `printf hi > out.txt`：PowerShell 5.1 的 `>` 写出 UTF-16，Windows 分支改用 `Set-Content -NoNewline`。④ 「shell 包装藏不住失败的测试」用 `ruby test || true`，PowerShell 5.1 没有 `||`；Windows 分支用等价的 `ruby test; exit 0`，同样吞掉退出码、同样带 shell 运算符。Unix 分支与所有断言不变。
- **Windows 上的 clippy 以前从没跑到过，这次补上。** Windows job 以前在 Test 步骤就挂了，走不到后面的 Clippy。用 `x86_64-pc-windows-gnu` 交叉跑 clippy 发现 `-D warnings` 下有三处失败：检查点恢复符号链接时 `target` 变量在非 Unix 上没用到；supervisor 测试的 `child_pid` 在 Windows 上只读取不检查；monitor 测试里只给 Unix 测试用的辅助函数在 Windows 上成了死代码。都改为按平台编译，行为不变。
...
### Added
- **Web 端轮次结束有明确信号。** 每轮结束后在聊天里落一行居中的灰色分隔线「── 本轮结束 · 总耗时 3.2m · 工具 12 次 · 1 失败 · 轮到你 ──」，与 TUI 同一条；工具数从运行卡片里数，会话重载后再追加，恢复的轮次不知道开始时间只报工具数。浏览器不响铃。
- **Web 输入框下方提示常驻「轮到谁」。** 跑的时候「本轮进行中 · 回车即送达 · 右侧按钮停止」（强调色），空闲时「轮到你 · Enter 发送 · Shift+Enter 换行」（绿色）。体验基线清单 Web 列第 4 至 8 项至此全部到位。

## [0.78.0-rc17] - 2026-09-20

### Changed
- **Web 运行卡片的折叠规则对齐 TUI。** 此前只按「最后 5 步」截断，一轮里的工具步和轮次标记混在一起。现在以「正在思考 · 第 N 轮」步为界分小轮次：早先的小轮次收成一行「第 N 轮 · k 步 · j 失败」，最新一轮留一行汇总加最新一步，正在跑的那步永远可见；外层仍按 5 行兜底显示「… 前面还有 N 步」。体验基线清单第 4 项 Web 列到位。

## [0.78.0-rc16] - 2026-09-20

### Added
- **Web 端也能看到思维链。** rc6 起 Runtime 已按段下发 `reasoning_delta`，Web 的 `client_event` 一直直接丢弃。现在原样放行文本，前端在运行卡片下方画一行灰色「思考中」尾巴（只留最后 240 字，空白折叠），正文增量、工具调用或新一轮一来即清，与 TUI 的「思考中 / 回复中」规则一致。体验基线清单第 5 项 Web 列到位。
