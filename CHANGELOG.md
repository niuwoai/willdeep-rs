...
- **Windows 上两条中断恢复测试改为仅 Unix。** `local_interrupted_write_resumes_without_replay` 和 `interrupted_foreground_child_is_discovered_resumed_and_reported_without_replay` 用 `scripts/lib/agent_eval_process.rb` 做中断控制器，这个脚本依赖进程组、fd 3/4 私有管道、按组 KILL，全是 POSIX 专有的；评测脚本本身也只在 Linux CI 上跑。所以这两条标 `#[cfg(unix)]`，断言不变。`monitor_events` 那条同理：给 monitor 的命令是 POSIX shell（`for` 循环加 `grep --line-buffered`），Windows 上 Shell 工具走 PowerShell；monitor 自己的单元测试也只在 Unix 上跑。
- **后台 supervisor 测试等 PID 文件的期限从 5 秒放宽到 20 秒，超时时打印 supervisor 的输出。** Windows 上被测命令要先起一个 PowerShell、再嵌套起一个 `powershell.exe` 才写 PID 文件，CI 上冷启动经常超过 5 秒（同仓库的 Windows Job Object 测试给的是 15 秒）。「父进程断开后 5 秒内收掉子进程」这条断言不变。以前超时只报「condition was not met」，看不出是慢还是命令出错。代价是：Windows 上「中断后恢复不重放」这条路径目前没有自动化覆盖。
